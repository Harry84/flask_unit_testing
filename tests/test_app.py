...
# Write a test to test Post functionality i.e. adding a new registree to the db

class TestAdd(TestBase):
    def test_home_post(self):
        response = self.client.post(url_for('home'), data = dict(name="Barkus Aurelius"), follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Barkus Aurelius', response.data)
